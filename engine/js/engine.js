...
var classes = {}, def_classes = {}, animations = {}, cclass = "", pageClasses = {}, pageDefClasses = {}, ffs = {}, f_c = {}, cps = {}
...
try {
    ...
    var avail_styles = { 'Border': 'Border', 'Border Top': 'BorderTop', 'Border Left': 'BorderLeft', 'Border Bottom': 'BorderBottom','Opacity': 'Opacity', 'Border Right': 'BorderRight', 'Animation': "Animation", 'Background blur': "Bgblur", 'Padding': "Padding", 'Margin': "Margin", 'Display as': "Display", 'Text Style': "TextStyle", 'Text Decoration': "TextDecorator", 'Background Color': "BackgroundColor", 'Flex Direction': "FlexDirection", 'Border Radius': "BorderRadius", 'Width': "Width", 'Height': "Height", 'Aling content': "AlignItems", 'Justify Content': "JustifyContent", 'Background Image (URL)': "BackgroundURL", 'Background Image (MEDIA)': "BackgroundMEDIA" }
    var avail_listeners = { 'Hover': "Hover", 'Focus': "Focus", 'Visited': "Visited", 'Target': "Target" }
    var style = [], media_data = []
    var tags = { "heading": "h1", "paragraph": "p", "input": "input", "button": "button", "section": "div", "Link": "a", "img": "img" }
    ...
    function set_up_fonts() {
        let f = null, fs = null, cid = null
        for (const [k, v] of Object.entries(elements)) {
            cid = k
            f = v.element.styles
            for (const [a, b] of Object.entries(f)) {

                if (b.style.type == "multiple") {

                    fs = b.style.options
                    for (const [x, y] of Object.entries(fs)) {
                        if (y.style.type == "fontfamily") {
                            let val = y.style.value
                            if (f_c[val] == null) {
                                f_c[val] = [cid,]
                                let stle = document.createElement("style")
                                stle.id = val
                                stle.innerHTML = `@import url('https://fonts.googleapis.com/css2?family=` + val + `&display=swap');`
                                if(location.pathname.includes("/download")){
                                    document.getElementById("style-fonts").insertAdjacentText("beforeend",stle.innerText)
                                }
        ...
        for (const [k, v] of Object.entries(def_classes)) {
            ...
            for (const [a, b] of Object.entries(f)) {
                    ...
                    for (const [x, y] of Object.entries(fs)) {
                        if (y.style.type == "fontfamily") {
                            ...
                            if (f_c[val] == null) {
                                ...
                                let stle = document.createElement("style")
                                stle.id = val
                                stle.innerHTML = `@import url('https://fonts.googleapis.com/css2?family=` + val + `:wght@400;500&display=swap');`
                                if(location.pathname.includes("/download")){
                                    document.getElementById("style-fonts").insertAdjacentText("beforeend",stle.innerText)
                                }
                                document.body.appendChild(stle)
                ...
                for (const [z, p] of Object.entries(ssf)) {
                    f = p.styles
                    for (const [a, b] of Object.entries(f)) {
    ...
    function load_page_data() {
        ...
        elements["live-page"].element.get_property()
    ...
    function list_anims(f = '') {
        ...
        for (const [k, v] of Object.entries(animations)) {
            if (k.toLowerCase().includes(f.toLowerCase())) {
                html += `<block style="cursor:pointer;" id="b-` + k + `"  onclick="load_anim('` + k + `');reset_keys();anim_switch_ui(1)">` + k + `</block>`
            }
        }
    ...
    function list_property_anim() {
        ...
        document.getElementById("menu-c-a").innerHTML = ''
        document.getElementById("menu-c-a").insertAdjacentHTML('beforeend', shtml)
    }
    ...
    function filter_fonts(data, cid, vl) {
        ...
        for (const [i, val] of Object.entries(ffs)) {
            if (i.toLowerCase().includes(vl.toLowerCase())) {
                if (val == data[0]) {
                    ft = val
                    ophtml += `<item class="active-menu" onclick="set_font('` + data[1] + `','` + data[2] + `','` + i + `','main-drop-` + data[3] + `');toggle_dropdown('drop-` + data[3] + `')" >` + i + `</item>`
                } else {
                    ophtml += `<item onclick="set_font('` + data[1] + `','` + data[2] + `','` + i + `','main-drop-` + data[3] + `');toggle_dropdown('drop-` + data[3] + `')">` + i + `</item>`
                }
            }
        }
    ...
    function filter_m_fonts(data, cid, vl) {
        let ophtml = ""
        for (const [i, val] of Object.entries(ffs)) {
            if (i.toLowerCase().includes(vl.toLowerCase())) {
                if (val == data[0]) {

                    ophtml += `<item class="active-menu" onclick="set_font('` + data[3] + `','` + data[1] + `','` + data[2] + `','` + i + `',` + data[4] + `,'main-drop-` + data[6] + `','` + data[5] + `');toggle_dropdown('drop-` + data[6] + `')" >` + i + `</item>`
                } else {
                    ophtml += `<item onclick="set_font('` + data[3] + `','` + data[1] + `','` + data[2] + `','` + i + `',` + data[4] + `,'main-drop-` + data[6] + `','` + data[5] + `');toggle_dropdown('drop-` + data[6] + `')">` + i + `</item>`
                }
            }
    ...
    function change_sub_style(o, id, name, val, an, hid) {
        if (!an) {
            ...
            animations[cur_anim].keyframes[cur_key][sp].style.options[name].style.value = val
            let d = animations[cur_anim].keyframes[cur_key][sp]
            reload_css_anim()
        }
    }
    ...
    function convert_m_data(o, id, name, val, an, t, hd) {
        ...
        let ol = document.getElementById(t).parentElement.children[2].children
        let nl = []
        for (const [i, va] of Object.entries(ol)) {
            if (va.innerText == val) {
                if (!va.classList.contains("active-menu")) {
                    va.classList.add("active-menu")
                }
                nl.push(va)
            }
        }
        for (const [i, va] of Object.entries(ol)) {
            if (va.innerText != val) {
                if (va.classList.contains("active-menu")) {
                    va.classList.remove("active-menu")
                }
                nl.push(va)
            }
        }
        document.getElementById(t).parentElement.children[2].innerHTML = ''
        for (const [i, va] of Object.entries(nl)) {
            document.getElementById(t).parentElement.children[2].insertAdjacentHTML("beforeend", va.outerHTML)
    ...
    function set_font(o, id, name, val, an, t, hid) {
        if (!an) {
            if (id != 'null') {
                let dv = elements[current].element.styles[o].style.options[name].style.value
                if (f_c[dv] != null) {
                    let ol = f_c[dv], nl = []
                ...
                if (hid == '' || hid == 'undefined') {
                    let dv = d.styles[o].style.options[name].value
                    if (f_c[dv] != null) {
                        let ol = f_c[dv], nl = []
                        for (const [k, v] of Object.entries(ol)) {
                            if (cclass != v) {
                    ...
                    let dv = d.listeners[hid].styles[o].style.options[name].value
                    if (f_c[dv] != null) {
                        let ol = f_c[dv], nl = []
                        for (const [k, v] of Object.entries(ol)) {
                            if (cclass + hid != v) {
    ...
    function change_style(id, name, val, an, t, hid = '') {
        if (!an) {
            ...
            animations[cur_anim].keyframes[cur_key][name].style.value = val
            let d = animations[cur_anim].keyframes[cur_key][name]
            if (d.style.ismu) {
                let c = d.style
                let inputs = d.style.options
                for (const [i, v] of Object.entries(inputs)) {
    ...
    function show_chooser(fr, id, name, an, t, hid) {
        if (cps[fr] == null) {
            let colorPicker = new iro.ColorPicker('picker#' + fr.split("-")[0] + "-picker", {
                width: 200,
                layout: [
                    {
            ...
            colorPicker.color.rgbaString = colorPicker.el.children[0].value
            colorPicker.el.children[0].oninput = () => { colorPicker.color.rgbaString = colorPicker.el.children[0].value }
            colorPicker.on('color:change', function (color) {
                colorPicker.el.children[0].value = color.rgbaString
                document.getElementById(fr).style.backgroundColor = color.rgbaString
                change_style(id, name, color.rgbaString, an, t, hid)

            });
            cps[fr] = colorPicker
        }
        document.getElementById(fr.split("-")[0] + "-picker").classList.toggle("hidden")
    }
    function show_m_chooser(fr, o, id, name, an, t, hid) {
        if (cps[fr] == null) {
            let colorPicker = new iro.ColorPicker('picker#' + fr.split("-")[0] + "-picker", {
                width: 200,
                layout: [
            ...
            colorPicker.on('color:change', function (color) {
                colorPicker.el.children[0].value = color.rgbaString
                document.getElementById(fr).style.backgroundColor = color.rgbaString
                change_sub_style(o, id, name, color.rgbaString, an, t, hid)
    ...
    function generate_css(currentPage) {
        ...
        var pageCSS = document.getElementById("style-fonts").innerText+ styles + lis_css + anim
        console.log(pageCSS)
        return pageCSS
    }
    ...
    document.addEventListener("click", function (event) {
        document.getElementsByTagName("menu")[0].style.display = "none";
        try {
            let menus = document.getElementsByTagName("dropdown-menu")
            for (const [k, v] of Object.entries(menus)) {
                if (event.target.tagName != "DROPDOWN" && !v.contains(event.target) && event.target.parentElement.tagName != "DROPDOWN") {
                    if (!v.classList.contains("hidden")) {
                        v.classList.add("hidden")
                    }
                }
            }
            if (event.target.tagName != "PAGE-DETIALS" && event.target.tagName != "PAB"
                && event.target.tagName != "P-DROP" && event.target.tagName != "PAGE-DETIALS") {
                let menus = document.getElementsByTagName("pages-dropdown")[0]
                if (!menus.classList.contains("hidden")) {
                    menus.classList.add("hidden")
                }

            }
            let cpsh = document.getElementsByTagName("picker")
            for (const [k, v] of Object.entries(cpsh)) {
                if (!v.contains(event.target) && event.target.tagName != "CHOOSER") {
                    if (!v.classList.contains("hidden")) {
                        v.classList.add("hidden")
                    }
                }
    ...
    function exit_fullscreen() {
        ...
        document.getElementsByClassName("en-below-bars")[0].style.display = "block"
    ...
    async function load_page_classes() {
        cclass = ""
        def_classes = {}
        ...
        load_page_anim().then(load_css_anim)
    }
        ...
        for (const [i, j] of Object.entries(pageAnim)) {
            ...
            for (const [k, v] of Object.entries(j.keyframes)) {
                ...
                for (const [x, y] of Object.entries(v)) {
                    let ns = null
                    ns = eval("new " + y.style.nclass + "(null)")
                    ns.style.value = y.style.value
                    if (ns.style.type == "multiple") {
                        for (const [c, d] of Object.entries(ns.style.options)) {
                            d.style.value = y.style.options[c].style.value
                        }
                    }

                    animations[i].keyframes[k][y.style.nclass] = ns
                }
    ...
    function reload_css_anim() {
        ...
        for (const [k, v] of Object.entries(keys)) {
            ...
            for (const [i, c] of Object.entries(v)) {
                ...
                if (c.style.type == "multiple") {
                    for (const [a, b] of Object.entries(c.style.options)) {
                        if (b.style.value != '') {
                            css_sub += b.style.get_css()
                        }
    ...
    function remove_style(id, c, hid = '', an) {
        if (id != "null") {
            ...
            if (!an) {
                ...
                delete animations[cur_anim].keyframes[cur_key][c]
                load_anim()
    ...
    class Style {
        ...
        propertyHTML(t = null, o = null, animat = false, hd = '') {
            ...
            if (t == null) {
                if (this.type == "dropdown") {
                    ...
                    for (const [i, val] of Object.entries(this.options)) {
                        if (val == value) {
                            ft = val
                            fhtml = `<item class="active-menu" onclick="convert_data('` + this.elemid + `','` + this.nclass + `','` + val + `',` + animat + `,'main-drop-` + id + `','` + hd + `');toggle_dropdown('drop-` + id + `')" >` + val + `</item>`
                        } else {
          ...
          <property-action>
          <p-action  onclick="remove_style('` + this.elemid + `','` + this.nclass + `','` + hd + `',` + animat + `)">
              remove Property
          </p-action>
      </property-action>
          `
                    ...
                    return `<property-div style="position:relative;justify-content: space-between;align-items: center;" class="property-container">
                    <property-h>` + this.title + `</property-h>
                    <chooser style="background-color:`+ this.value + `;" onclick="show_chooser(this.id,'` + this.elemid + `','` + this.nclass + `',` + animat + `,'','` + hd + `')" id="` + hd + id + this.title.replace(" ", "") + `-chooser"></chooser>
                    <picker class="hidden" id="`+ hd + id + this.title.replace(" ", "") + `-picker"><input value="` + this.value + `" placeholder="RGBA value"></picker>
                    </property-div>
                ...
                <dropdown-menu  class="property-dropdown hidden" id='drop-` + id + `' value="` + this.value + `" >
                <input oninput="filter_fonts(['`+ this.value + `','` + this.elemid + `','` + this.nclass + `','` + id + `'],'cont-` + id + `',this.value)" placeholder='Search fonts'>
        ...
        `
          ...
          `
                ...
                if (this.type == "dropdown") {
                    ...
                    let fhtml = ''
                    for (const [i, val] of Object.entries(this.options)) {
                        let list = [this, o, animat, id, hd]
                        if (val == this.value) {
                            ft = val
                            fhtml = `<item class="active-menu" onclick="convert_m_data('` + o + `','` + this.elemid + `','` + this.nclass + `','` + val + `',` + animat + `,'main-drop-` + id + `','` + hd + `');toggle_dropdown('drop-` + id + `')" >` + val + `</item>`
                        } else {
                            ophtml += `<item  onclick="convert_m_data('` + o + `','` + this.elemid + `','` + this.nclass + `','` + val + `',` + animat + `,'main-drop-` + id + `','` + hd + `');toggle_dropdown('drop-` + id + `')">` + val + `</item>`
                        }
                    }
                    html = `
          ...
          `
                    ...
                    let ophtml = ""
                    let dffs = ffs
                    for (const [k, v] of Object.entries(dffs)) {
                        if (k == value) {
                            ft = k
                            ophtml += `<item class="active-menu" onclick="set_font('` + o + `','` + this.elemid + `','` + this.nclass + `','` + k + `',` + animat + `,'main-drop-` + id + `','` + hd + `');toggle_dropdown('drop-` + id + `')" >` + k + `</item>`
                        } else {
                            ophtml += `<item onclick="set_font('` + o + `','` + this.elemid + `','` + this.nclass + `','` + k + `',` + animat + `,'main-drop-` + id + `','` + hd + `');toggle_dropdown('drop-` + id + `')">` + k + `</item>`
                        }
                    }
                    html = `
                ...
                <dropdown-menu  class="property-dropdown hidden" id='drop-` + id + `' value="` + this.value + `" >
                <input oninput="filter_m_fonts(['`+ this.value + `','` + this.elemid + `','` + this.nclass + `','` + o + `','` + animat + `','` + hd + `','` + id + `'],'cont-` + id + `',this.value)" placeholder='Search fonts'>
                <dropdown-cont id='cont-` + id + `'>
              `+ ophtml + `
              </dropdown-cont>
              </dropdown-menu>
        ...
        <h3>` + this.title + `</h3>
        ` + html + `
        <p class="en-del-prop" onclick="remove_style('` + this.elemid + `','` + this.nclass + `','` + hd + `',` + animat + `)">Remove Style</p>
        </div>
        ...
        `
                    ...
                    return `<property-div style="position:relative;justify-content: space-between;align-items: center;" class="property-container">
                    <property-h>` + this.title + `</property-h>
                    <chooser onclick="show_m_chooser(this.id,'` + o + `','` + this.elemid + `','` + this.nclass + `',` + animat + `,'` + hd + `')" id="` + hd + t + this.title.replace(" ", "") + `-chooser"></chooser>
                    <picker class="hidden" id="`+ hd + t + this.title.replace(" ", "") + `-picker"><input></picker>
                    </property-div>
                    ...
                    html = `
              <input id="`+ hd + t + this.title.replace(" ", "") + `" placeholder="` + this.title + `" type="` + this.type + `" oninput="change_sub_style('` + o + `','` + this.elemid + `','` + this.nclass + `',this.value,` + animat + `,'` + hd + `')" value="` + this.value + `">
              `
                    return `<property-div>
              <property-h>` + this.title + `</property-h>
              ` + html + `
        ...
        set_listen_style() {
            ...
            if (classes[this.clas] == null) {
                if (location.pathname.includes("download")) {
                    ...
                    style = "live " + tags[this.clas.split('-')[0]] + ':' + this.title.toLowerCase() + '{' + css + '}'
                ...
                if (location.pathname.includes("download")) {
                    ...
                    style = "live  ." + this.clas + ':' + this.title.toLowerCase() + '{' + css + '}'
    ...
    class Animation {
        constructor(elemid, value = "", title = "Animation") {
            this.style = new Style(this.constructor.name, elemid, title, "multiple", value, "js",
                {
                    "AnimationName": new AnimationName(elemid),
                    "AnimationDuration": new AnimationDuration(elemid),
                    "AnimationDelay": new AnimationDelay(elemid),
                    "AnimationDirection": new AnimationDirection(elemid),
                    "AnimationFillMode": new AnimationFillMode(elemid),
                    "AnimationIteration": new AnimationIteration(elemid),
                    "AnimationState": new AnimationState(elemid),
                    "AnimationFunction": new AnimationFunction(elemid),
                })
        }
    }

    class AnimationName {
        constructor(elemid, value = "", title = "Name") {
            this.style = new Style(this.constructor.name, elemid, title, "text", value, "animationName")
        }
    }
    class AnimationDuration {
        constructor(elemid, value = "", title = "Duration") {
            this.style = new Style(this.constructor.name, elemid, title, "text", value, "animationDuration")
        }
    }
    class AnimationDelay {
        constructor(elemid, value = "", title = "Delay") {
            this.style = new Style(this.constructor.name, elemid, title, "text", value, "animationDelay")
        }
    }
    class AnimationDirection {
        constructor(elemid, value = "", title = "Direction") {
            this.style = new Style(this.constructor.name, elemid, title, "dropdown", value, "animationDirection", ["normal", "reverse", "alternate", "alternate-reverse"])
        }
    }
    class AnimationFillMode {
        constructor(elemid, value = "", title = "Store Style") {
            this.style = new Style(this.constructor.name, elemid, title, "dropdown", value, "animationFillMode", ["normal", "forwards", "backwards", "both"])
        }
    }
    class AnimationIteration {
        constructor(elemid, value = "", title = "Repeat") {
            this.style = new Style(this.constructor.name, elemid, title, "number", value, "animationIterationCount")
        }
    }
    class AnimationState {
        constructor(elemid, value = "running", title = "State") {
            this.style = new Style(this.constructor.name, elemid, title, "dropdown", value, "animationPlayState", ["paused", "running"])
        }
    }
    class AnimationFunction {
        constructor(elemid, value = "running", title = "Function") {
            this.style = new Style(this.constructor.name, elemid, title, "dropdown", value, "animationTimingFunction", ["linear", "ease", "ease-in", "ease-in-out"])
        }
    }
    ...
    class MarginTop {
        constructor(elemid, value = "", title = "Top") {
            this.style = new Style(this.constructor.name, elemid, title, "text", value, "marginTop")
        }
    }
    class MarginRight {
        constructor(elemid, value = "", title = "Right") {
            this.style = new Style(this.constructor.name, elemid, title, "text", value, "marginRight")
        }
    }
    class MarginLeft {
        constructor(elemid, value = "", title = "Left") {
            this.style = new Style(this.constructor.name, elemid, title, "text", value, "marginLeft")
        }
    }

    class Width {
        constructor(elemid, value = "", title = "Width") {
            this.style = new Style(this.constructor.name, elemid, title, "text", value, "width")
        }
    }

    class Height {
        constructor(elemid, value = "", title = "Height") {
            this.style = new Style(this.constructor.name, elemid, title, "text", value, "height")
        }
    }

    class BorderRadius {
        constructor(elemid, value = "", title = "Border Radius") {
            this.style = new Style(this.constructor.name, elemid, title, "text", value, "borderRadius")
        }
    }

    class Display {
        constructor(elemid, value = "", title = "Display") {
            this.style = new Style(this.constructor.name, elemid, title, "dropdown", value, "display", ["block", "none", "flex", "inline"])
        }
    }

    class TextColor {
        constructor(elemid, value = "#000000", title = "Font Color") {
            this.style = new Style(this.constructor.name, elemid, title, "color", value, "color")
        }

    }
    class TextDecoration {
        constructor(elemid, value = "none", title = "Decoration") {
            this.style = new Style(this.constructor.name, elemid, title, "dropdown", value, "textDecoration", ["none", "underline", "overline", "line-through",])
        }

    }
    class TextDecorationStyle {
        constructor(elemid, value = "none", title = "Style") {
            this.style = new Style(this.constructor.name, elemid, title, "dropdown", value, "textDecorationStyle", ["solid", "double", "dotted", "dashed", "wavy",])
        }

    }
    class TextDecorationColor {
        constructor(elemid, value = "none", title = "Color") {
            this.style = new Style(this.constructor.name, elemid, title, "dropdown", value, "textDecorationStyle", ["solid", "double", "dotted", "dashed", "wavy",])
        }

    }
    class TextDecorationWeight {
        constructor(elemid, value = "", title = "Weight") {
            this.style = new Style(this.constructor.name, elemid, title, "text", value, "textDecorationThickness")
        }
    ...
    class FontFamily {
        constructor(elemid, value = "Roboto", title = "Family") {
    ...
    class TextDecorator {
        constructor(elemid, value = '', title = 'Text Decoration') {
            this.style = new Style(this.constructor.name, elemid, title, 'multiple', value, 'js',
                {
                    "TextDecoration": new TextDecoration(elemid),
                    "TextDecorationStyle": new TextDecorationStyle(elemid),
                    "TextDecorationColor": new TextDecorationColor(elemid),
                    "TextDecorationWeight": new TextDecorationWeight(elemid)
                })
        }
    }
    ...
    class Opacity {
        constructor(elemid, value = "", title = "Opacity") {
            this.style = new Style(this.constructor.name, elemid, title, "number", value, "opacity")
        }
    }
